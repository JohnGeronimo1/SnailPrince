...
7. Now to test the app with the command: node app
8. To stop a live server type ctrl c in the terminal window
-Sharon
*/
/*******************************************************/
...
const CreateUrl = require('./util').CreateUrl;                  //The ./ means look for util in a different 
const Create_IMDB_URL = require('./util').Create_IMDB_URL;      //location at the same current level.
    ...
    async function get_database_imdb_url(){
        ...
        let query= db.query(sql,(error, result) =>{
            ...
            {   console.log("\n IMDB_url is null\n\n");
                        ...
                        var sql = `SELECT imdbID FROM oscar_winner_data_csv WHERE entity = '${title}' `; 
                        let query= db.query(sql,(error, result) =>{
                        if(error) throw error;
...
//Uses: Strictly for developers only. Users would not have access to this funtion.
//Searchs for the matching entry in the database based on the year. It then deletes the corresponding rows.
...
});//End of request to gather all data in csv table.


//Singleton endpoint for year.
//Used to return 1 the row that has matching a year in the csv table in the database.
...
*/
 ...
 });//End of Get requests for year.
    

//Used to return all the elements that have matching a year in the csv table in the database. (Its a collection endpoint)
...
});//End of Get request for category.
...
app.get('/api/database/movies/winners/:winner',(req,res)=>{                 
    database = new Database (config);
        let params = [`${req.body.winner}`]; //User can only provide the year,the category,the name of the entity, whether an award is won.
        let sql = `SELECT * FROM oscar_winner_data_csv WHERE winner = ?`; //Grabs all the matching winners.
    database.query(sql,params).then(result => {
        console.log(result);
        res.send(result);
    database.close();
    });//End of query.
});//End of Get request for winners. 
...
});//End of Get request for entities.
...
app.get('/api/database/movies/years/:year1/:year2',(req, res) => {
    ...
    */
    database = new Database (config);
        let params = [req.params.year1, req.params.year2];
...
});//End of Get request for date range function
